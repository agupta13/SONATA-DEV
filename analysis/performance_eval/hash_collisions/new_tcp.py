...
def get_filter_query(packets_fname, qid, ref_level, Th=[]):
    out = ''
    if qid == 2:
        out = '(load_rdd(packets_fname, sc).map(lambda s: ((s[0], str(IPNetwork(str(str(s[3])+"/%s")).network)), 1))' \
...
def analyse_query(fname, sc):
    ...
    spark_queries = {}
    query_out = {}
    query_2_percentile_thresh = {2: 99.99}
    query_2_actual_thresh = {}
    refinement_levels.sort(reverse=True)

    """
    Start with /32 find the query output after filter
    use that output to join with intermediate query out filters to retain those tuples
    and go reverse
    """
    out_packets = {}
    for qid in qids:
                ...
                if qid == 2:
                    ...
                    if ref_level < refinement_levels[0]:
                        ...
                        refined_satisfied_out = (out_filter
                                                 .map(lambda s: ((s[0][0], str(IPNetwork(str(str(s[0][1]) + "/" + str(ref_level))).network)), 1))
    ...
    for qid in qids:
        ...
        for ref_level in refinement_levels:
            ...
            out_packets[qid][transit] = {}
            for partid in [partitioning_plans[qid][0], partitioning_plans[qid][-1]]:
                if qid == 2:
                    Th = [query_2_actual_thresh[2][ref_level]]
                ...
                if partid == 0:
                    tmp_out = tmp_transit_rdd.map(lambda s: s[0][1]).collect()
                    out_packets[qid][transit][partid] = tmp_out
            ...
            for ref_level_curr in refinement_levels:
                if ref_level_prev < ref_level_curr:
                    ...
                    for partid in [partitioning_plans[qid][0], partitioning_plans[qid][-1]]:
                        ...
                        # print qid, (ref_level_prev, ref_level_curr), partid, tmp_count_rdd.take(2)

                        if partid == 0:
                            tmp_out = tmp_count_rdd.map(lambda s: s[0][1]).collect()
                            out_packets[qid][transit][partid] = tmp_out

    # print query_count_transit
    # print out_packets[2][(0,32)][0][:2]
    return out_packets
