...
def reduce_size(fname, out_fname):
    ...
    transformed_data = (transformed_data
                        .map(lambda s: tuple(list(s[:7]) + ['.', '.'] + list(s[9:11]) + get_tcp_flag(s[11]) + list(s[-2:])))
                        )
    print transformed_data.take(5)

    """
    ts,sIP,sPort,dIP,dPort,nBytes,proto,sMac,dMac,tcp.seq,tcp.ack,tcp.flags,tcp.hdr_len,tcp.window_size
    """
