...
def get_filter_query(qid, ref_level, Th=[]):
    out = ''
    if qid == 111:
        out = '(packets.map(lambda s: ((s[0], str(IPNetwork(str(str(s[3])+"/%s")).network), s[2]), 1))' \
              '.reduceByKey(lambda x, y: x + y))' % (str(ref_level))

    elif qid == 113:
        out = '(packets.map(lambda s: ((s[0], str(IPNetwork(str(str(s[3])+"/%s")).network), s[2]), 1))' \
              '.reduceByKey(lambda x, y: x + y)).filter(lambda s: s[1] > %d).join(' \
              'packets.map(lambda s: (s[0], (str(IPNetwork(str(str(s[1])+"/%s")).network), s[4]), 1))' \
...
def get_spark_query(qid, ref_level, part, Th=[]):
    out = ''
    if qid == 112:
        if part == 0:
            out = '(packets.map(lambda s: ((s[0], str(IPNetwork(str(str(s[1])+"/%s")).network), s[4]), 1)))' % (str(ref_level))

        else:
            out = '(packets.map(lambda s: ((s[0], str(IPNetwork(str(str(s[1])+"/%s")).network), s[4]), 1)).reduceByKey(lambda x, y: x + y))' % (
                str(ref_level))
    elif qid == 111:
        if part == 0:
            out = '(packets.map(lambda s: ((s[0], str(IPNetwork(str(str(s[3])+"/%s")).network), s[2]), 1)))' % (str(ref_level))
...
def analyse_query(fname):
    ...
    spark_queries = {}
    query_out = {}
    query_2_percentile_thresh = {111: 95, 113: 95}
    ...
    #             if qid == 111:
    #                 tmp_query = get_filter_query(qid, ref_level)
    #                 print tmp_query
    ...
    #                                              .map(
    #                         lambda s: ((str(IPNetwork(str(str(s[0][0]) + "/" + str(ref_level))).network), s[0][1]), 1))
    #                                              .reduceByKey(lambda x, y: x + y)
    #                                              )
    ...
    #                     post_filter_query = get_spark_query(qid, ref_level, '1', Th=[thresh])
    ...
    #
    #             elif qid == 113:
    #                 tmp_query = get_filter_query(qid, ref_level, Th=[query_2_actual_thresh[111][ref_level]])
    #                 print tmp_query
    #                 if ref_level < refinement_levels[0]:
    #                     out_filter = load_rdd(final_query_out[qid], sc)
    #                     print out_filter.take(5)
    ...
    #
    #                     filter_out_fname = "filter_out_" + str(qid) + "_" + str(ref_level)
    #                     post_filter_query = get_spark_query(qid, ref_level, '1',
        ...
        for ref_level in refinement_levels:
            ...
            for partid in partitioning_plans[qid]:
                if qid == 113:
                    ...
                    Th = []
                tmp_query = get_spark_query(qid, ref_level, partid, Th)
            ...
            for ref_level_curr in refinement_levels:
                if ref_level_prev < ref_level_curr:
                    ...
                    for partid in partitioning_plans[qid]:
                        ...
                        tmp_count_rdd = (curr_level_rdd.map(lambda s: (str(IPNetwork(str(str(s[0][1])+"/"+str(ref_level_prev))).network), s))
